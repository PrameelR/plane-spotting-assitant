...
class EditPlaneSpotting extends Component {
    static displayName = EditPlaneSpotting.name;
    ...
    async componentDidUpdate(prevProps, prevState) {
        if (prevProps.isShowEditPlaneSpotting !== this.props.isShowEditPlaneSpotting) {
            if (this.props.isShowEditPlaneSpotting) {
                await this.onClickClearEditPlaneSpotting();
            }
        }
    }
    ...
    setDate = (e) => {
        ...
        _state.value = e;
        _state.isvalid = e ? true : false;
        _state.validationmessage = _state.isvalid ? "" : "Please select date";
        this.setState({
            date: _state
        });
    ...
    onClickSubmitEditPlaneSpotting = () => {
        ...
        if (planemake.isvalid && planemodel.isvalid && plane.isvalid && location.isvalid && date.isvalid && image.isvalid) {
            ...
            this.props.EditPlaneSpotting(data).then(result => {
                var updateplanespottingstatus = this.props.updateplanespottingstatus;
                openNotification("Edit plane spotting", updateplanespottingstatus.message, updateplanespottingstatus.code === 1 ? "success" : "error");
                this.props.HideLoading();
            });
    ...
    setPlaneSpottingData = () => {
        var selectedplanespotting = this.props.selectedplanespotting;


        var bufferarray = this.base64ToArrayBuffer(selectedplanespotting.image);
        var blob = new Blob([new Uint8Array(bufferarray)]);
        var blobUrl = URL.createObjectURL(blob);

        this.setState({
            planemake: { value: selectedplanespotting.planemakeid, isvalid: true, isrequired: true, validationmessage: "Please select plane make" },
            planemodel: { value: selectedplanespotting.planemodelid, isvalid: true, isrequired: true, validationmessage: "Please select plane model" },
            plane: { value: selectedplanespotting.planeid, isvalid: true, isrequired: true, validationmessage: "Please select plane" },
            location: { value: selectedplanespotting.location, isvalid: true, isrequired: true, validationmessage: "Please enter location" },
            date: { value: moment(selectedplanespotting.date), isvalid: true, isrequired: true, validationmessage: "Please select date" },
            image: { value: bufferarray, isvalid: true, isrequired: true, validationmessage: "Please upload image", selectedFile: null, selectedFileList: [], selectedFileName: "", url: blobUrl },
        });
        this.getPlaneMakes();
        this.getPlaneModels(selectedplanespotting.planemakeid);
        this.getPlanes(selectedplanespotting.planemakeid, selectedplanespotting.planemodelid);
    }
    ...
    render() {
        ...
        return (
            <>
                <Modal title={<h5 className="modal-title">Edit plane spotting</h5>} visible={this.props.isShowEditPlaneSpotting} onCancel={e => this.props.CloseEditPlaneSpotting()} footer={null}>
                    <div className="row w-100 d-flex flex-column modal-inputs p-0 m-0">
                        ...
                        <div className="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 p-2 pt-4 d-flex justify-content-around">
                            <Button type="submit" className="btn-custom-primary w-50 me-2 d-flex justify-content-center align-items-center " type="primary" onClick={this.onClickSubmitEditPlaneSpotting}>Submit</Button>
                            <Button type="submit" className="btn-custom-primary w-50 ms-2 d-flex justify-content-center align-items-center " type="default" onClick={this.onClickClearEditPlaneSpotting}>Clear</Button>
                        </div>
                    </div>
                </Modal>
...
const
    mapStateToProps = state => {
        return {
            ...
            updateplanespottingstatus: state.plane.updateplanespottingstatus,
...
const
    mapDispatchToProps = dispatch => {
        return {
            ...
            CloseEditPlaneSpotting: () => dispatch(actions.CloseEditPlaneSpotting()),
            EditPlaneSpotting: (data) => dispatch(actions.UpdatePlaneSpottingAction(data)),
            SearchPlaneSpottings: (data) => dispatch(actions.SearchPlaneSpottings(data))

        };
    };

export default connect(mapStateToProps, mapDispatchToProps)(EditPlaneSpotting);