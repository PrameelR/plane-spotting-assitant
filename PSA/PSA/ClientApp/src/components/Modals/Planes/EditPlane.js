...
class EditPlane extends Component {
    static displayName = EditPlane.name;
    ...
    componentDidUpdate(prevProps, prevState) {
        if (prevProps.isShowEditPlane !== this.props.isShowEditPlane) {
            if (this.props.isShowEditPlane) {
                this.onClickClearEditPlane();
            }
        }
    }
    ...
    onClickSubmitEditPlane = () => {
        ...
        if (referenceprefix.isvalid && referencesuffix.isvalid && planemake.isvalid && planemodel.isvalid && image.isvalid) {
            ...
            this.props.EditPlane(data).then(result => {
                var updateplanestatus = this.props.updateplanestatus;
                openNotification("Edit plane ", updateplanestatus.message, updateplanestatus.code === 1 ? "success" : "error");
                this.props.HideLoading();
            });
    ...
    setPlaneData = () => {
        ...
        var bufferarray = this.base64ToArrayBuffer(selectedplane.image);
        var blob = new Blob([new Uint8Array(bufferarray)]);
        var blobUrl = URL.createObjectURL(blob);

        this.setState({
            reference: { value: selectedplane.reference, isvalid: true, isrequired: true, validationmessage: "Please enter valid reference", regex: /^[A-Za-z0-9\b]+$/ },
            referenceprefix: { value: selectedplane.referenceprefix, isvalid: true, isrequired: true, validationmessage: "Please enter valid reference", regex: /^[A-Za-z0-9\b]+$/ },
            referencesuffix: { value: selectedplane.referencesuffix, isvalid: true, isrequired: true, validationmessage: "Please enter valid reference", regex: /^[A-Za-z0-9\b]+$/ },
            planemake: { value: selectedplane.planemakeid, isvalid: true, isrequired: true, validationmessage: "Please select plane make" },
            planemodel: { value: selectedplane.planemodelid, isvalid: true, isrequired: true, validationmessage: "Please select plane model" },
        ...
        this.getPlaneMakes();
        this.getPlaneModels(selectedplane.planemakeid);
    }
    ...
    render() {
        ...
        return (
            <>
                <Modal title={<h5 className="modal-title">Edit plane </h5>} visible={this.props.isShowEditPlane} onCancel={e => this.props.CloseEditPlane()} footer={null}>
                    <div className="row w-100 d-flex flex-column modal-inputs p-0 m-0">
                        ...
                        <div className="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 p-2 p-0 m-0">
                            ...
                            >
                                {this.state.image.url ? <div className="w-100 h-100  d-flex align-items-center justify-content-center">
                                    <img src={this.state.image.url} alt="avatar" className="upload-plane-img w-auto h-auto" />
                                </div>
                        ...
                        <div className="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 p-2 pt-4 d-flex justify-content-around">
                            <Button type="submit" className="btn-custom-primary w-50 me-2 d-flex justify-content-center align-items-center " type="primary" onClick={this.onClickSubmitEditPlane}>Submit</Button>
                            <Button type="submit" className="btn-custom-primary w-50 ms-2 d-flex justify-content-center align-items-center " type="default" onClick={this.onClickClearEditPlane}>Clear</Button>
                        </div>
                    </div>
                </Modal>
...
const
    mapStateToProps = state => {
        return {
            ...
            updateplanestatus: state.plane.updateplanestatus,
...
const
    mapDispatchToProps = dispatch => {
        return {
            ...
            CloseEditPlane: () => dispatch(actions.CloseEditPlane()),
            EditPlane: (data) => dispatch(actions.UpdatePlaneAction(data)),
